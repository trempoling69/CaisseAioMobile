    ...
    if (
      ...
      try {
        const { data } = await axiosInstance.post(
          '/api/authentification/refresh/tokens',
          {},
          { withCredentials: true },
        );
...
const handleRequestError = (error: AxiosError<GenericRequestError>) => {
  ...
  if (error.response) {
    ...
    throw new Error(
      `Erreur sur la configuration de la requête : ${error.message}`,
    );
  }
};
...
const get = async <T>(
  url: string,
  params: any = {},
): Promise<GenericRequestResponse<T>> => {
  try {
...
const post = async <T, D>(
  ...
  try {
    const response = await axiosInstance.post<GenericRequestResponse<T>>(
      url,
      data,
      { headers: await requestHeader() },
    );
...
const del = async <T>(url: string): Promise<GenericRequestResponse<T>> => {
  try {
    const response = await axiosInstance.delete<GenericRequestResponse<T>>(
      url,
      { headers: await requestHeader() },
    );
    return handleReponse<GenericRequestResponse<T>>(response);
...
const put = async <T, D>(
  ...
  try {
    const reponse = await axiosInstance.put<GenericRequestResponse<T>>(
      url,
      data,
      { headers: await requestHeader() },
    );
...
const patch = async <T, D>(
  ...
  try {
    const reponse = await axiosInstance.patch<GenericRequestResponse<T>>(
      url,
      data,
      { headers: await requestHeader() },
    );
